...
import './style.css';
import { quoteCount, findQuotes, relatedQuotes, posterUrl, prepareVideo } from './data.js';
...
function App() {
  const [related, setRelated] = useState([]), [aiStatus, setAiStatus] = useState(''), [aiError, setAiError] = useState('');
  ...
  async function runSearch(value, append = false) {
    ...
    if (!append) {
      ...
      const aiController = new AbortController(); aiRequest.current = aiController;
      setRelated([]); setAiError(''); setAiStatus('loading');
      relatedQuotes(trimmed, aiController.signal).then(found => {
        if (!aiController.signal.aborted) { setRelated(found); setAiStatus('done'); }
      }).catch(e => {
        if (!aiController.signal.aborted) { setAiError(e.message); setAiStatus('error'); }
      });
    }
  ...
  return <div className="app">
    ...
    <main>
      <section className={`hero ${searched ? 'compact' : ''}`}>
        ...
        <form className="search-box" onSubmit={e => { e.preventDefault(); runSearch(input); }}><Icon name="search" size={24}/><input ref={inputRef} aria-label="搜索中英文台词" placeholder="输入词语、句子或场景，找一句台词…" value={input} maxLength={120} onChange={e => setInput(e.target.value)}/>{input && <button className="clear" type="button" aria-label="清空搜索" onClick={reset}><Icon name="close" size={17}/></button>}<button className="search-submit" type="submit" disabled={!input.trim()}>寻找台词 <Icon name="arrow" size={18}/></button></form>
        <div className="suggestions"><span>试着搜</span>{['leave me alone', '喜欢', 'beautiful'].map(word => <button key={word} onClick={() => runSearch(word)}>{word}<span>↗</span></button>)}</div>
      </section>
      {!searched ? <section className="collection"><div className="section-top"><h2>从这一部开始</h2><span>THE FIRST CHAPTER — 01</span></div><div className="film-card"><div className="film-art" aria-label="梧桐树与落日插画"><div className="sun"/><div className="hill back"/><div className="hill front"/><div className="tree"><span className="trunk"/><span className="crown one"/><span className="crown two"/><span className="crown three"/></div><span className="art-caption">FLIPPED</span><span className="art-year">a story about seeing things differently.</span></div><div className="film-info"><span className="film-label">本期电影 / 2010</span><h3>怦然心动 <span>Flipped</span></h3><p>从一棵梧桐树、一双明亮的眼睛开始，<br/>在朱莉和布莱斯的故事里，听懂心动与成长。</p><div className="film-tags"><span>{count ? `${count.toLocaleString()} 句` : '中英'}双语台词</span><span>原声片段</span></div><button className="explore" onClick={() => runSearch('beautiful')}>找一句 beautiful <Icon name="arrow" size={18}/></button></div></div><div className="steps"><span><b>01</b> 搜索一句表达</span><span><b>02</b> 在场景中听懂</span><span><b>03</b> 再听一遍，记住它</span></div></section> : <section className="results" aria-label="台词搜索结果"><div className="section-top"><h2>{loading ? '正在寻找…' : <>关于「{query}」的台词 <span className="result-count">{total}</span>{semanticBadge}</>}</h2><span>怦然心动 · FLIPPED (2010)</span></div><div aria-live="polite">{error && <div className="empty" role="alert"><h3>搜索遇到了一点问题</h3><p>{error}</p><button onClick={() => runSearch(query)}>重新搜索</button></div>}{loading ? <div className="skeleton" role="status">正在电影中寻找这句表达…</div> : !error && rows.length === 0 && related.length === 0 ? (aiStatus === 'loading' ? <div className="empty" role="status"><span className="empty-icon">“ ”</span><h3>正在寻找意思相近的台词…</h3><p>没有找到原文完全一致的表达，AI 正在按含义匹配。</p></div> : <div className="empty"><span className="empty-icon">“ ”</span><h3>没有找到贴合这个意思的台词</h3><p>换一种说法试试，或从一个更具体的词开始。</p><button onClick={() => runSearch('喜欢')}>试试「喜欢」 <Icon name="arrow" size={16}/></button></div>) : null}</div>
        ...
        {related.length > 0 && <div className="semantic-divider" role="note"><span>以下是意思相近的台词</span>AI 按含义匹配，可能与你的理解不同</div>}
        {related.map(row => <article className={`quote-card ${active === row.id ? 'selected' : ''}`} key={row.id}>
          <Player row={row} active={active} onActivate={setActive}/>
          <div className="quote-details"><div className="quote-meta"><span>意思相近</span><time>{timestamp(row.start)}</time></div>
          <div className="quote-content"><div><p className="en" tabIndex={0} title={row.english}>{row.english}</p><p className="zh" tabIndex={0} title={row.chinese}>{row.chinese}</p></div></div></div>
          <p className="match-reason" title={row.reason}>{row.reason}</p>
        </article>)}</div>
